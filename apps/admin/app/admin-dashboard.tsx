...
type Section = "overview" | "detailers" | "customers" | "rewards" | "payments" | "payouts" | "jobs" | "finance" | "issues" | "documents" | "data" | "reconciliation" | "policies" | "audit" | "team";
...
type Dashboard = {
  ...
  affiliatePoints: Array<{ id: string; detailerId: string; detailerName: string; detailerEmail: string; customerId: string; customerName: string; customerEmail: string; bookingId: string; points: number; reason: "first_referred_booking_completed"; createdAt: string }>;
};
...
const sectionMeta: Record<Section, { label: string; short: string; description: string }> = {
  ...
  rewards: { label: "Affiliate points", short: "AP", description: "Live referral points and audit trail" },
...
const navGroups: { title: string; items: Section[] }[] = [
  { title: "Operations", items: ["overview", "detailers", "customers", "jobs", "rewards"] },
...
function recordsFor(section: Section, dashboard: Dashboard): RecordItem[] {
  ...
  if (section === "rewards") return dashboard.affiliatePoints.map((entry) => ({ id: entry.id, title: `${entry.detailerName} · +${entry.points} points`, subtitle: `${entry.customerName} (${entry.customerEmail})`, meta: date(entry.createdAt), status: "Awarded once", tone: "good", detail: `Awarded after the referred customer's first completed booking. Booking ${entry.bookingId}. Detailer ${entry.detailerEmail}. No cash commission is generated.` }));
...
function Overview({ dashboard, onOpen }: { dashboard: Dashboard; onOpen: (section: Section) => void }) {
  ...
  return <div className="dashboard">
    <section className="metric-grid"><Metric label="Real customers" value={String(metrics.customers)} change="Smoke accounts excluded"/><Metric label="Approved detailers" value={`${metrics.approvedDetailers}/${metrics.detailers}`} change="Approved / real accounts"/><Metric label="Booking requests" value={String(metrics.bookings)} change={`${metrics.activeBookings} currently active`}/><Metric label="Affiliate points" value={String(metrics.affiliatePointsAwarded)} change="10 per first completed referral"/></section>
