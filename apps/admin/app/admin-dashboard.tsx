...
const customers: RecordItem[] = [
  { id: "CUS-1842", title: "Sample Customer", subtitle: "customer@example.invalid", meta: "2 vehicles · 8 bookings", status: "Active", tone: "good", detail: "Phone •••• 0123 · Water available · Last detail 21 July 2026 · No open complaints" },
...
const jobs: RecordItem[] = [
  { id: "JOB-7731", title: "Full Detail · Range Rover Evoque", subtitle: "Jordan Miles → Reece Thomson", meta: "29 Jul · 10:00–12:34 · OX10", amount: "£120.00", status: "Evidence complete", tone: "good", detail: "3 arrival photos · 4 completion photos · One pre-existing wheel blemish recorded · Customer rating pending" },
...
const documents: RecordItem[] = [
  ...
  { id: "DOC-577", title: "Jordan Miles · Insurance", subtitle: "Approved 20 Jul 2026", meta: "PDF · expires 14 May 2027", status: "Approved", tone: "good", detail: "Approved by owner@example.invalid. Review reminder scheduled 30 days before expiry." },
];
...
const auditLog: RecordItem[] = [
  { id: "AUD-9321", title: "Admin MFA verified", subtitle: "owner@example.invalid", meta: "29 Jul 2026 · 14:02 · Session A19", status: "Recorded", tone: "good", detail: "Approved owner account completed the second-factor challenge. Session timeout set to 15 minutes of inactivity." },
  { id: "AUD-9318", title: "Insurance document approved", subtitle: "DOC-577 · Jordan Miles", meta: "29 Jul 2026 · 13:41", status: "Recorded", tone: "good", detail: "Approval performed by owner@example.invalid. Previous record retained; expiry reminder created." },
...
export default function AdminDashboard({ signedInEmail, signOutHref }: { signedInEmail: string; signOutHref: string }) {
  ...
  const [admins, setAdmins] = useState([{ email: "owner@example.invalid", role: "Owner", status: "Active" }]);
  ...
  const exportRows = () => {
    ...
    link.href = url;
    link.download = `valx-${section}-prototype.csv`;
    link.click();
...
function Overview({ onOpen }: { onOpen: (section: Section) => void }) {
  return <div className="dashboard">
    ...
    <section className="panel activity-panel"><PanelHead eyebrow="LATEST ACTIVITY" title="Recent service events" />
      ...
      ] as const).map(([time, title, detail, tone]) => <div key={time + title}><time>{time}</time><i className={tone} /><span><strong>{title}</strong><small>{detail}</small></span></div>)}</div>
    </section>
  </div>;
}
...
function TeamAccess({ admins, setAdmins, pending, approve }: { admins: { email: string; role: string; status: string }[]; setAdmins: (value: { email: string; role: string; status: string }[]) => void; pending: { email: string; requested: string }[]; approve: (email: string) => void }) {
  ...
  return <div className="team-page">
    <section className="access-banner"><span>LOCK</span><div><small>SERVER-SIDE ALLOWLIST</small><h2>Approved accounts only</h2><p>The sample owner account is <strong>owner@example.invalid</strong>. Portal records never expose full bank or card credentials.</p></div></section>
