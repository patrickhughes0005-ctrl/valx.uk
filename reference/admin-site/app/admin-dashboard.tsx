...
const customers: RecordItem[] = [
  { id: "CUS-1842", title: "Reece Thomson", subtitle: "reecetomo@hotmail.co.uk", meta: "2 vehicles · 8 bookings", status: "Active", tone: "good", detail: "Phone •••• 0123 · Water available · Last detail 21 July 2026 · No open complaints" },
...
const documents: RecordItem[] = [
  ...
  { id: "DOC-577", title: "Jordan Miles · Insurance", subtitle: "Approved 20 Jul 2026", meta: "PDF · expires 14 May 2027", status: "Approved", tone: "good", detail: "Approved by reecetomo@hotmail.co.uk. Review reminder scheduled 30 days before expiry." },
];
...
const auditLog: RecordItem[] = [
  { id: "AUD-9321", title: "Admin MFA verified", subtitle: "reecetomo@hotmail.co.uk", meta: "29 Jul 2026 · 14:02 · Session A19", status: "Recorded", tone: "good", detail: "Approved owner account completed the second-factor challenge. Session timeout set to 15 minutes of inactivity." },
  { id: "AUD-9318", title: "Insurance document approved", subtitle: "DOC-577 · Jordan Miles", meta: "29 Jul 2026 · 13:41", status: "Recorded", tone: "good", detail: "Approval performed by reecetomo@hotmail.co.uk. Previous record retained; expiry reminder created." },
...
export default function AdminDashboard({ signedInEmail, signOutHref }: { signedInEmail: string; signOutHref: string }) {
  ...
  const [admins, setAdmins] = useState([{ email: "reecetomo@hotmail.co.uk", role: "Owner", status: "Active" }]);
...
function TeamAccess({ admins, setAdmins, pending, approve }: { admins: { email: string; role: string; status: string }[]; setAdmins: (value: { email: string; role: string; status: string }[]) => void; pending: { email: string; requested: string }[]; approve: (email: string) => void }) {
  ...
  return <div className="team-page">
    <section className="access-banner"><span>LOCK</span><div><small>SERVER-SIDE ALLOWLIST</small><h2>Approved accounts only</h2><p>The initial approved account is <strong>reecetomo@hotmail.co.uk</strong>. Portal records never expose full bank or card credentials.</p></div></section>
